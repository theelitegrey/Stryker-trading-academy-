  ...
  CURRENT_ADMIN_DOCS.forEach((admin) => {
    ...
    row.innerHTML =
      '<div class="cell-user">' + (typeof avatarImgHtml === 'function' ? avatarImgHtml(admin.uid, (admin.displayName || admin.email), null, 36) : '<div class="cell-avatar" style="background:linear-gradient(135deg,var(--teal),var(--teal-dim));"></div>') + '<div><span class="cell-name">' +
        stkEsc(admin.displayName || admin.email || 'Unnamed') + (isSelf ? ' <span class="status-tag active" style="margin-left:6px;">You</span>' : '') +
        '</span><span class="cell-sub">' + stkEsc(admin.email || '—') + '</span></div></div>' +
      ...
      (!isSelf ? '<button class="btn btn-ghost btn-sm" data-revoke-uid="' + stkEsc(admin.uid) + '">Revoke</button>' : '');
