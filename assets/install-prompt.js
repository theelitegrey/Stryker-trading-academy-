  ...
  function openSheet(){
    ...
    wrap.innerHTML =
      ...
      '<div class="install-sheet" role="dialog" aria-modal="true" aria-label="Install Stryker">' +
        ...
        (ios && !canPrompt ? iosStepsHtml() : '') +
        '<div class="install-sheet-actions">' +
          (canPrompt
            ? '<button type="button" class="btn btn-primary" id="install-go">Install the app</button>'
            : (ios ? '' : '<button type="button" class="btn btn-primary" id="install-go">Install the app</button>')) +
          '<button type="button" class="btn btn-ghost" data-install-close>' + (ios && !canPrompt ? 'Got it' : 'Not now') + '</button>' +
        '</div>' +
    ...
    if (go) go.addEventListener('click', function () {
      if (!deferredEvent) {
        // Chromium hasn't offered install here (already installed elsewhere,
        // or criteria not met yet) — say so instead of a dead button.
        if (typeof showToast === 'function') showToast('info', 'Your browser is not offering install right now — try the browser menu → "Add to Home screen".');
        closeSheet();
        return;
      }
      ...
      ev.userChoice.then(function (choice) {
        ...
        else { deferredEvent = ev; closeSheet(); }
      });
    });
  }
